...
export default function ArmarPresupuestoPage() {
  ...
  async function agregar(apu) {
    // congelar el costo de la APU = suma de sus líneas
  ...
  const calc = (base, cant) => {
    ...
    return {
      ...
      valorUnit,
      valorTotal: valorUnit * cant,
    };
  };

  const totalCosto = lineas.reduce((s, l) => s + calc(l.costo_unitario, l.cantidad).costoTotal, 0);
  const totalValor = lineas.reduce((s, l) => s + calc(l.costo_unitario, l.cantidad).valorTotal, 0);
              ...
              return (
                <tr key={l.id} className="border-t hover:bg-gray-50">
                  <td className="px-3 py-2 font-medium">
                    {l.apus?.codigo} · {l.apus?.descripcion}
                  </td>
                  ...
                  <td className="px-3 py-2 text-right font-medium" style={{ color: "#00369C" }}>
                    {formato(r.valorTotal)}
                  </td>
